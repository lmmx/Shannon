...
# Determines whether the algorithm will make effort to find unique
# sparsest flow when decomposing a node.
worry_abt_unique = 0
...
debug_mode = 0
# Set this to 1 if you want to overload the variable normalization to the
# copy_count of the true
overwrite_normalization = 0
# if overwrite_normalization=1: equivalent to using original Copy counts
# for thresholding
n_inp = len(sys.argv)
comp = ''
sample_name = ''
...
reconstr_file = sample_name_out + \
    'algo_output/reconstructed_comp_' + str(comp) + '.fasta'
...
def single_nodes_to_fasta():
    # The function outputs individual nodes without any edges as reconstructed
    # transcripts.
    ...
    for line in lines:
        ...
        if i != 0:
            ...
            for hashcode in tokens:
                ...
                if paths_for_node.get(node) is None:
...
def ParseEdgeFile(EdgeFile, graph):
    ...
    for line in lines:
        if i != 0:
            ...
            start_node.out_edges.append(
                [end_node, int(tokens[2]), float(tokens[3]), float(tokens[4])])
            end_node.in_edges.append(
                [start_node, int(tokens[2]), float(tokens[3]), float(tokens[4])])
        i += 1
    f.close()
...
# Edge object (used for building copycount filter matrices).
class Edge(object):
    ...
    def __init__(self, start_node, end_node, overlap_weight, weight, L):
        ...
        # How much the connected nodes overlap.
        self.overlap_weight = overlap_weight
        self.weight = weight  # The copycount for the edge.
        # The normalization used for error term in copycount filtering.
        self.L = L
...
class Node(object):  # Node object (used universally)
    ...
    def __init__(self, node_string, node_weight, L, name):
        # The sequence of bases that the node represents.
        self.string = node_string
        # A list of edges in which this node object is the end node.
        self.in_edges = []
        # A list of edges in which this node object is the start node.
        self.out_edges = []
        self.name = name  # Hash value for the node.
        self.weight = (node_weight)  # The copycount for the node.
        # The normalization used for the error term in copycount filtering.
        self.L = (L)
        # Start position in the reference DNA at which this node has bases from
        self.DNA_start_pos = None
        # Start position in the reference DNA at which this node has bases
        # from.
...
class Graph(object):  # Graph object (used universally)
    ...
    def __init__(self):
        ...
        self.paths = []
        # for matrix
        # This is used for building the matrix with the node's in/out edge
        # information.
        self.edges = []
        # for edge filter
        self.edges2 = []
        ...
        # Is there a unique sparsest flow for each decomposed node in the
        # graph?
        ...
        # dictionary with nodes as keys and nodes that were condensed to form
        # node as values
    ...
    def findStartAndEnd2(self):
        ...
        # Adds a dummy node labeled end_node that has an in edge to all
        # original nodes with out degree 0.
    ...
    def printNodes(self):
        # Prints out each node with all in-edges and out-edges on the same
        # line.
        print('Nodes:\n', [[e.name, e.weight, e.L] for e in self.nodes])
        print('\n')
        for each in self.nodes:
            if len(each.out_edges) == 0:
                list1 = [[each.in_edges[i][0].name, each.in_edges[i][1], each.in_edges[
                    i][2], each.in_edges[i][3]] for i in range(0, len(each.in_edges))]
                print(each.name, "   out edges: None", "   in edges:", list1)
            if len(each.in_edges) == 0:
                list1 = [[each.out_edges[i][0].name, each.out_edges[i][1], each.out_edges[
                    i][2], each.out_edges[i][3]] for i in range(0, len(each.out_edges))]
                print(each.name, "   out edges:", list1, "   in edges: None")
            if len(each.out_edges) != 0 and len(each.in_edges) != 0:
                list_out = [[each.out_edges[i][0].name, each.out_edges[i][1], each.out_edges[
                    i][2], each.out_edges[i][3]] for i in range(0, len(each.out_edges))]
                list_in = [[each.in_edges[i][0].name, each.in_edges[i][1], each.in_edges[
                    i][2], each.in_edges[i][3]] for i in range(0, len(each.in_edges))]
                print(
                    each.name,
                    "   out edges:",
    ...
    def printNodesSmall(self):
        ...
        for each in self.nodes:
            if len(each.out_edges) != 0:
                list_out = [[each.out_edges[i][0].name]
                            for i in range(0, len(each.out_edges))]
                print(each.name, "   out edges:", list_out)

    def findEdges(self):
        # finds all edges in the graph and updates all the relevant data
        # structures.
        for node in self.nodes:
    ...
    def filter_update(self, new_edge_weights):
        ...
        for edge in self.edges2:
            edge[0].out_edges.append(
                [edge[1], edge[2], new_edge_weights[i], edge[4]])
            edge[1].in_edges.append(
                [edge[0], edge[2], new_edge_weights[i], edge[4]])
            i += 1
    ...
    def filter_update_incnodes(self, new_weights, m, n):
        ...
        for edge in self.edges2:
            if overwrite_normalization:
                # overwrite normalization with true copy-count of the original
                # edge
                edge[0].out_edges.append(
                    [edge[1], edge[2], new_weights[i], edge[3]])
                edge[1].in_edges.append(
                    [edge[0], edge[2], new_weights[i], edge[3]])
            else:
                edge[0].out_edges.append(
                    [edge[1], edge[2], new_weights[i], edge[4]])
                edge[1].in_edges.append(
                    [edge[0], edge[2], new_weights[i], edge[4]])
            i += 1
        ct = 0
        for node in self.nodes:
    ...
    def search(self):
        ...
        for node in self.nodes:
            if (len(node.in_edges) != 0) and (len(node.out_edges) != 0) and (
                    node is not self.start) and (node is not self.end):
                if len(
                        node.in_edges) > 1 or len(
                        node.out_edges) > 1:  # SEARCH FOR any Y nodes
                    if use_Y_paths and len(
                            node.in_edges) <= 1:  # Search for left-Y NODES and X-nodes
                        continue
                    self.tobereduced.append(node)
        self.tobereduced.sort(
            key=lambda x: int(
                x.name.split("_")[0]),
            reverse=False)

    def algorithm2(self):
        # Runs sparse flow algorithm on graph to simplify graph such that all
        # nodes have an in-degree of 1.
        done = False
        cycle_limit = 1
        ...
        while not done:
            ...
            # Algorithm will construct new list of nodes for each iteration
            # through all nodes
            ...
            for node in self.nodes:
                ...
                sys.stdout.write('comp: ' +
                                 ...
                                 str(node.name) +
                                 ', m: ' +
                                 str(len(node.in_edges)) +
                                 ', n: ' +
                                 str(len(node.out_edges)))
                sys.stdout.flush()
                if 1:
                    if 1:
                        # list of new nodes produced from decomposition of the
                        # current node.
                        new_nodes = []
                        # A vector that contains the connected node of each
                        # in-edge.
                        inedges = []
                        # A vector that contains the connected node of each
                        # out-edge.
                        outedges = []
                        # A vector that contains the copycounts of each
                        # in-edge.
                        inedge_vector = []
                        # A vector that contains the copycounts of each
                        # in-edge.
                        outedge_vector = []
                        # A vector that should contain the copycounts of each
                        # in node, but currently contains the overlap of the
                        # sequence.
                        inedge_cc = []
                        # A vector that should contain the copycounts of each
                        # out node, but currently contains the overlap of the
                        # sequence.
                        outedge_cc = []

                        # A dictionay that contains the overlap and
                        # normalization information for each in edge.
                        incoming_edge_attributes = {}
                        # A dictionay that contains the overlap and
                        # normalization information for each out edge.
                        outgoing_edge_attributes = {}
                        if len(node.in_edges) == 0:
                            print('Hanging Node!')
                            node.in_edges.append(
                                [self.start, 0, node.weight, 0])
                            self.start.out_edges.append(
                                [node, 0, node.weight, 0])
                            self.start.weight += float(node.weight)

                        if len(node.out_edges) == 0:
                            print('Hanging Node!')
                            node.out_edges.append(
                                [self.end, 0, node.weight, 0])
                        ...
                        # This section of code determines which known paths
                        # will be considered when decomposing this node
                        path_bridge_dict = {}
                        paths_for_all = []
                        if node in paths_for_node:
                            for kp1 in paths_for_node.get(node):
                                kp1_nodes = known_paths[kp1]
                                if kp1_nodes[0] in self.constituent_nodes[
                                        node] or kp1_nodes[-1] in self.constituent_nodes[node]:
                        ...
                        if paths_for_node.get(node) is not None:
                            for (m, in_node) in enumerate(inedges):
                                if paths_for_node.get(in_node) is None:
                                    continue
                                for (n, out_node) in enumerate(outedges):
                                    if paths_for_node.get(out_node) is None:
                                        continue

                                    node_paths_temp = [
                                        paths_for_node[node1] for node1 in self.constituent_nodes[node]]
                                    node_paths = []
                                    ...
                                    cand_paths = intersect5(
                                        paths_for_node[
                                            self.constituent_nodes[in_node][
                                                -1]],
                                        node_paths,
                                        paths_for_node[
                                            self.constituent_nodes[out_node][0]],
                                        paths_for_node[in_node],
                                        paths_for_node[out_node])
                                    l_node = self.constituent_nodes[in_node]
                                    r_node = self.constituent_nodes[out_node]
                                    c_node = self.constituent_nodes[node]
                                    for cp in cand_paths:
                                        node_list = known_paths[
                                            cp]  # for this path
                                        if self.constituent_nodes[node][
                                                0] in node_list and self.constituent_nodes[node][-1] in node_list:
                                            tmp1 = [
                                                node_list.index(n1) for n1 in self.constituent_nodes[node]]
                                            node_good = True
                                            ...
                                            if node_good:
                                                ...
                                                l_good = True
                                                r_good = True
                                                l_check = min(tmp1[0], len(
                                                    self.constituent_nodes[in_node]))
                                                r_check = min(
                                                    len(node_list) - tmp1[-1] - 1, len(self.constituent_nodes[out_node]))
                                                for y in range(0, l_check):
                                                    if node_list[
                                                            tmp1[0] - 1 - y].string != l_node[-1 - y].string:
                                                        l_good = False
                                                for y in range(0, r_check):
                                                    if node_list[
                                                            tmp1[-1] + 1 + y].string != r_node[y].string:
                                                        r_good = False
                        ...
                        # This section of the code builds the new nodes formed
                        # during decomposition, and implicitly condenses the
                        # 1x1 nodes
                        for i in range(0, m):
                            for j in range(0, n):
                                ...
                                if curr_edge_cc != 0:
                                    ...
                                    in_1new = in_node_flow[
                                        i] == temp_matrix[i][j]
                                    out_1new = out_node_flow[
                                        j] == temp_matrix[i][j]
                                    if in_node_out_deg == 1 and in_1new and out_node_in_deg == 1 and out_1new:
                                        in_node = inedges[i]
                                        out_node = outedges[j]
                                        out_attr = outgoing_edge_attributes[
                                            outedges[j]]
                                        in_attr = incoming_edge_attributes[
                                            inedges[i]]
                                        new_node = Node(
                                            in_node.string[
                                                :-
                                        ...
                                        for in_edge in in_node.in_edges:
                                            ...
                                            in_edge[0].add_out_edge(
                                                new_node, in_edge[1], in_edge[2], in_edge[3])
                                        # Make all new edges for new node.
                                        for out_edge in out_node.out_edges:
                                            new_node.out_edges.append(out_edge)
                                            out_edge[0].add_in_edge(
                                                new_node, out_edge[1], out_edge[2], out_edge[3])

                                        self.nodes.append(new_node)
                                        self.constituent_nodes[new_node] = self.constituent_nodes[
                                            in_node] + self.constituent_nodes[node] + self.constituent_nodes[out_node]
                                        ...
                                        new_node = Node(
                                            in_node.string[
                                                :-in_attr[0]] + node.string,
                                            curr_edge_cc,
                                            node.L,
                                            node.name + "_[" + str(i) + "," + str(j) + "]")
                                        # Make all new edges for new node.
                                        for in_edge in in_node.in_edges:
                                            new_node.in_edges.append(in_edge)
                                            in_edge[0].add_out_edge(
                                                new_node, in_edge[1], in_edge[2], in_edge[3])
                                        new_node.add_out_edge(
                                            outedges[j], out_attr[0], temp_matrix[i][j], out_attr[1])
                                        outedges[j].add_in_edge(
                                            new_node, out_attr[0], temp_matrix[i][j], out_attr[1])

                                        self.nodes.append(new_node)
                                        self.constituent_nodes[new_node] = self.constituent_nodes[
                                            in_node] + self.constituent_nodes[node]
                                        new_nodes.append(new_node)
                                        new_node_list.append(new_node)
                                        ...
                                        for out_edge in out_node.out_edges:
                                            ...
                                            out_edge[0].add_in_edge(
                                                new_node, out_edge[1], out_edge[2], out_edge[3])
                                        new_node.add_in_edge(
                                            inedges[i], in_attr[0], temp_matrix[i][j], in_attr[1])
                                        inedges[i].add_out_edge(
                                            new_node, in_attr[0], temp_matrix[i][j], in_attr[1])

                                        self.nodes.append(new_node)

                                        self.constituent_nodes[new_node] = self.constituent_nodes[
                                            node] + self.constituent_nodes[out_node]
                                        new_nodes.append(new_node)
                                        new_node_list.append(new_node)
                                        ...
                                        self.constituent_nodes[
                                            new_node] = self.constituent_nodes[node]
                                        new_nodes.append(new_node)
                                        new_node_list.append(new_node)
                        ...
                        # For each node that was condensed into a new node,
                        # delete all it's connections.
    ...
    def read_paths_recursive(self, node, str_till_now, overlap, prev_weight):
        ...
        # This assumes all paths end at the _END node.
        if len(node.out_edges) == 0:
    ...
    def read_paths(self):
        # reads paths in the case when there are only 1x1 nodes (ndoes with at
        # most one in edge AND at most one out edge).
...
def buildMatrixIncNodes(graph):
    ...
    for edge in graph.edges:
        ...
        if edge.end is not graph.end:
            ...
            A[node_count + node_indices_other[edge.end],
                edge_indices_other[edge]] = -1.
...
def filter_copycounts_inc_nodes(graph):
    ...
    if run_penalized:
        # this is the real value of penality
        pen_cost = pen_constant * abs(opt_val) / sum(x)
