...
def analyzer_blat(Repeats_File, Dest_File, exp_file, N):
    ...
    with open(Dest_File, 'w') as write_file:
        ...
        if len(best_rec_dict):
            write_file.write("#Average fractional contig of transcripts retrived:" +
                             '\t' + str(perf / len(best_rec_dict)) + '\n')
        else:
            write_file.write(
                "#Average fractional contig of transcripts retrived:" +
                '\t' +
                str(perf) +
                '  best_rec=0' +
                '\n')
        if sum_len > 0:
            write_file.write("#Sum of positions retreived / sum of lengths:" +
                             '\t' + str(float(sum_ret) / float(sum_len)) + '\n')
    ...
    with open(Dest_File, 'w') as write_file:
        ...
        if len(best_rec_dict):
            ...
            write_file.write(
                ...
                '\n')
        write_file.write(
            "# of transcripts at greater than 90%:" +
            '\t' +
            str(no_tr_rec))
        ...
        if tokens[0] == ">" or i == len(lines) - 1:
            if not_first_time:
                ...
                if len(sorted_list) > 0:
                    best_rec.append([identity, sorted_list[0][0], int(
                        sorted_list[0][1]), tr_len, tr_ab / tr_len])
                else:
                    best_rec.append(
                        [identity, 'None', 0, tr_len, tr_ab / tr_len])
            not_first_time = 1
...
def false_positive(Rec_fasta, LongReads_rec_per, Dest_File):
    ...
    for lines in open(Rec_fasta):
        ...
        # print(lines)
        if tokens[0][0] != '>':
            clen = tr_dict.get(curr_name, [0, 0])
            clen = clen[1]
...
def performance_plot(reconstr_log, trinity_log, plot_file, L, S, N):
    ...
    for lines in open(reconstr_log):
        ...
        if fields[0][0] != '#' and float(fields[3]) > S:  # fragment size
            ...
            tr_len = float(fields[3])
            tr_ab = float(fields[4])
    ...
    with open(plot_file, 'w') as plotFile:
        plotFile.write(
            'Abundance \t No. Trans \t Frac. reconstructed (Ours) \t Frac. reconstructed (Trinity) \t Bases reconstructed (Ours) \t Bases reconstr (Trinity) \t No bases\n')
        for (i, ab) in enumerate(ab_list):
            plotFile.write(str(ab) +
                           '\t' +
                           str(no[i]) +
                           '\t' +
                           str(frac[i]) +
...
def extract_isoforms(fasta_file, mummer_file, out_file):
    ...
    with open(out_file, 'w') as plotFile:
        ...
        for (tr, val) in tr_dict.items():
            plotFile.write(str(tr) + '\t0\t0\t0\t0\t' +
                           str(val) + '\t' + str(val_dict[val]) + '\n')


def performance_plot_express(
        oracle_fasta,
        num_isoform_file,
        reconstr_log,
        trinity_log,
    ...
    if exp_format == 'EXP':
        ...
        for lines in open(exp_file):
            ...
            try:
                ...
                tr_ab = float(fields[6]) / N
                tr_cov = float(fields[6]) / \
                    max(1, tr_len - L) * L  # * 20 / 135
                tot_ab = tot_ab + tr_ab
                # last 4 entries are num_iso, ours,trinity,cufflinks
                tr_dict_full[tr_id] = [tr_len, tr_cov, tr_ab, 0, 0, 0, 0]
            except ValueError:
                continue
        ...
        for lines in open(exp_file):
            fields = lines.strip().split()
            tr_id = 'hg19_wgEncodeGencodeBasicV17_' + fields[0].upper()
            tr_id = tr_id.upper()
            if not fields[1].isdigit():
                continue
            try:
                tr_len = float(fields[1])
                tr_ab = float(fields[7])
                tr_cov = float(fields[7])  # * 20 / 135
                tot_ab = tot_ab + tr_ab
                # last 4 entries are num_iso, ours,trinity,cufflinks
                tr_dict_full[tr_id] = [tr_len, tr_cov, tr_ab, 0, 0, 0, 0]
            except ValueError:
        ...
        for (tr_id, vec) in tr_dict_full.iteritems():
            tr_dict_full[tr_id] = [vec[0], vec[1] /
                                   tot_ab *
                                   N *
                                   L /
                                   max(1, vec[0]), vec[2] /
                                   tot_ab, vec[3], vec[4], vec[5], vec[6]]
    elif exp_format == 'KAL':
        tot_ab = 0
        for lines in open(exp_file):
            fields = lines.strip().split()
...
def reverse_performance_plot(
    ...
    with open(plot_file, 'a') as plotFile:
        ...
        plotFile.write(
            'iTrans reconstructed ' +
            str(no) +
            ' transcripts, and out of this, ' +
            str(rec) +
            ' of them have 0.95 overlap with real transcripts. \n')  # ' Fraction = ' + str(rec/no) + '\n')
        plotFile.write(
            'Trinity reconstructed ' +
            str(trinity_no) +
            ' transcripts, and out of this, ' +
            str(trinity_rec) +
        ...
        plotFile.write(
            'iTrans reconstructed ' +
            str(rec_bases) +
            ' bases correctly out of ' +
            str(bases) +
            ' total bases in reconstructed transcripts. \n')  # ' Fraction = ' + str(rec/no) + '\n')
        plotFile.write(
            'Trinity reconstructed ' +
            str(trinity_rec_bases) +
            ' bases correctly out of ' +
            str(trinity_bases) +
            ' total bases in reconstructed transcripts. \n')  # ' Fraction = ' + str(rec/no) + '\n')
...
def reverse_analyzer_blat(Repeats_File, Dest_File, fasta_file, N):
    ...
    for (i, line) in enumerate(lines):
        ...
        tokens = line.split()
        org = tokens[9]
        rec = tokens[13]  # use the threshold
        rec_len = int(tokens[0])
        tr_len = int(tokens[10])
        z = best_rec_dict.get(org, [None, 0])
        if rec_len > z[1]:
...
def reverse_analyzer(Repeats_File, Dest_File, fasta_file, N):
    ...
    for lines in open(fasta_file):
        ...
        if fields[0][0] == '>':
            ...
            # pdb.set_trace()
            continue
        tr_lengths[identity] = tr_lengths.get(
            identity, 0) + int(len(fields[0]))
        #tr_abundance[fields[0]] = float(fields[7])
        #sum_exp += float(fields[7])
        #sum_len += int(fields[1])
    # print(tr_lengths)
    ...
    for (i, line) in enumerate(lines):
        ...
        if tokens[0] == ">":
            ...
            if not_first_time:
                ...
                # raw_input()
                if len(sorted_list) > 0:
                    best_rec.append([identity, sorted_list[0][
                        0], sorted_list[0][1], tr_len])
                else:
